...
function HistoricoDupla() {
  const location = useLocation();
  const navigate = useNavigate();
  
  const duplaData = location.state;
  const duplaId = duplaData?.id;
  const nomeDupla = duplaData?.nome;
  ...
  useEffect(() => {
    if (abaAtiva === 'mapa-calor' && duplaId) {
      fetchMapaCalor();
    }
  }, [tipoAcaoSelecionada, abaAtiva, duplaId]);

  const fetchEstatisticas = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/estatisticas/dupla/${duplaId}/completo`);
      setEstatisticas({
        id: duplaId,
        nome: nomeDupla,
        ...response.data
  ...
  const fetchMapaCalor = async () => {
    try {
      ...
      const response = await api.get(
        `/estatisticas/dupla/${duplaId}/mapa-calor-posicoes?tipo_acao_id=${tipoAcaoSelecionada}`
      );
  ...
  const fetchHistoricoPartidas = async () => {
    try {
      ...
      const response = await api.get(`/estatisticas/dupla/${duplaId}/historico-partidas`);
  ...
  const fetchEstatisticasVitorias = async () => {
    try {
      ...
      const response = await api.get(`/estatisticas/dupla/${duplaId}/estatisticas-vitorias`);
    ...
    <div className="container mx-auto p-4 min-h-screen">
      ...
      <div className="relative mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-center text-gray-200 pt-12 md:pt-0">
          Histórico da Dupla - {nomeDupla}
        </h1>
        <p className="text-center text-gray-400 mt-2">
          Todas as partidas da dupla
        </p>
      </div>
