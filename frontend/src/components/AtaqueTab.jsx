...
import { getEstatisticasAtaqueAtleta, getMapaCalorAtleta } from '../services/estatisticas';
import MapaCalorQuadra from './MapaCalorQuadra';

/**
 * Aba de análise de Ataque
 */
function AtaqueTab({ atletaId, partidaId = null }) {
  ...
  const loadData = async () => {
    try {
      ...
      const [statsData, mapaData] = await Promise.all([
        getEstatisticasAtaqueAtleta(atletaId, partidaId),
        getMapaCalorAtleta(atletaId, 2, partidaId) // 2 = ID do tipo de ação "Ataque"
      ]);
      setStats(statsData);
      setMapaCalor(mapaData);
    } catch (error) {
      console.error('Erro ao carregar dados de ataque:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <p className="text-gray-400">Carregando dados de ataque...</p>
      </div>
    );
  }
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">Nenhum dado de ataque disponível</p>
      </div>
    );
  }
      ...
      <div className="bg-[#1E1E1E] rounded-xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-[#2a2a2a] rounded-lg p-4">
            ...
            <p className="text-3xl font-bold text-green-500">{stats.pontos}</p>
          </div>
          <div className="bg-[#2a2a2a] rounded-lg p-4">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-blue-900/30 rounded-lg p-4 border-l-4 border-[#00A3FF]">
            ...
            <p className="text-xs text-gray-500 mt-1">
              (Pontos - Erros) / Tentativas × 100
            </p>
          </div>
          <div className="bg-[#2a2a2a] rounded-lg p-4">
            <p className="text-sm text-gray-400 mb-1">Média por Partida</p>
            <p className="text-2xl font-bold text-white">
              {stats.tentativas > 0 ? (stats.pontos / (stats.tentativas / 10)).toFixed(1) : '0'}
            </p>
            <p className="text-xs text-gray-500 mt-1">Pontos de ataque</p>
          </div>
        </div>
      </div>
