...
from src.pontuacao.services import cadastra_ponto, cadastra_acao, obter_por_id, obter_acoes_partida_id, obtem_acoes, volta_ponto, atualiza_acao, atualiza_ponto
from src.pontuacao.schemas import Ponto, Acao, AcaoUpdate, AcaoResposta, PontoUpdate
...
@pontuacao_router.patch("/ponto/{ponto_id}")
def atualizar_ponto(ponto_id: int, ponto_update: PontoUpdate, db: Session = Depends(get_db)):
    return atualiza_ponto(ponto_id=ponto_id, ponto_update=ponto_update, db=db)
